...
public class Chunk {
    ...
    private void h(boolean flag) {
        ...
        if (this.world.areChunksLoaded(this.cachedArea, 16)) { // Use cache for BlockPosition
            for (int i = 0; i < 16; ++i) {
                for (int j = 0; j < 16; ++j) {
                    if (this.g[i + j * 16]) {
                        this.g[i + j * 16] = false;
    ...
    public void m() {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            for (int i1 = 0; i1 < 16; ++i1) {
                BlockPosition blockposition1 = this.blockPos.a(k, (j << 4) + i1, l); // Use cache for BlockPosition
    ...
    public void n() {
        ...
        if (!this.world.worldProvider.o()) {
            if (this.world.areChunksLoadedBetween(this.blockPos.a(-1, 0, -1), this.blockPos.a(16, this.world.F(), 16))) { // Use cache for block position
                label42:
                for (int i = 0; i < 16; ++i) {
                    for (int j = 0; j < 16; ++j) {
                        if (!this.e(i, j)) {
                ...
                if (this.lit) {
                    ...
                    for (EnumDirection enumdirection : EnumDirection.EnumDirectionLimit.HORIZONTAL) {
                        ...
                        this.world.getChunkAtWorldCoords(this.blockPos.shift(enumdirection, k)).a(enumdirection.opposite()); // Use cache for block position
                    }
